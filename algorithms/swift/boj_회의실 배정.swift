...
// BOJ_11000, 회의실 배정
// 비슷한 문제를 풀어본적이 있어서 힙을 사용하면 되겠다를 빨리 알았음
// 최솟값을 계속 갱신해주는 힙을 만들어서 다음 강의의 시작시간과 비교하면 된다
func solution() -> Int {
    let N = Int(readLine()!)!

    var lectures: [(Int, Int)] = []
    for _ in 0..<N {
        let item = readLine()!.split(separator: " ").compactMap { Int($0) }
        lectures.append((item[0], item[1]))
    }
    
    lectures = lectures.sorted(by: { lhs, rhs in
